...
const MILESTONE_REQUIREMENTS = {
  none: { level: 1, requirement: 2 },
  five: { level: 5, requirement: 5 },
  ten: { level: 10, requirement: 15 },
  twentyfive: { level: 25, requirement: 30 },
  fifty: { level: 50, requirement: 60 },
  hundred: { level: 100, requirement: 100 },
};
...
export function MilestoneLevel({ milestone, skillLevels }: MilestoneLevelProps) {
  ...
  const getNextRequiredLevels = () => {
    ...
    const [, { requirement }] = nextMilestone;
  ...
  return (
    <div className="space-y-2">
      <div className="flex items-center gap-2">
        <Badge 
          variant="secondary" 
          className={`${MILESTONE_COLORS[milestone]} flex items-center gap-2`}
        >
        ...
        {nextMilestone && (
          <TooltipProvider>
            <Tooltip>
              ...
              <TooltipContent>
                <p>Next milestone requirements:</p>
              </TooltipContent>
            </Tooltip>
          </TooltipProvider>
        )}
